...
for tensors, that is derivatives of Hessians.

These three types are used internally by the package. The user is not required to instantiate them, since the interface
operates at a higher level only requiring to define the function to be differentiated.
...
higher-order derivatives. One the other hand, the dual-based approach is thought to be faster than the type-based one 
