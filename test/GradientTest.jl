...
for f in TestFuncs.VECTOR_TO_SCALAR_FUNCS
    ...
    for c in (default_value(KWARG_DEFAULTS, :chunk), div(XLEN, 2), div(XLEN, 2) + 1, XLEN)
        ...
        g1_cache = ForwardDiff.@gradient(f; chunk = c, input_length = XLEN, cache=cache)
        g1! = ForwardDiff.@gradient(f; chunk = c, mutates = true)
        g1_cache! = ForwardDiff.@gradient(f; chunk = c, mutates = true, cache=cache)
        g2 = ForwardDiff.@gradient(f; chunk = c, allresults = true)
        g2_cache = ForwardDiff.@gradient(f; chunk = c, allresults = true, cache=cache)
        g2! = ForwardDiff.@gradient(f; chunk = c, input_length = XLEN, allresults = true, mutates = true)
        g2_cache! = ForwardDiff.@gradient(f; chunk = c, input_length = XLEN, allresults = true, mutates = true, cache=cache)
