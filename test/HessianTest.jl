...
for T in (StaticArrays.SArray, StaticArrays.MArray)
    ...
    @test_broken DiffResults.value(result1) == DiffResults.value(result)
    @test DiffResults.value(result2) == DiffResults.value(result)
    @test DiffResults.value(result3) == DiffResults.value(result)
    @test_broken DiffResults.gradient(result1) == DiffResults.gradient(result)
    ...
    @test_broken DiffResults.hessian(result1) == DiffResults.hessian(result)
