...
#----------------------#
+{N,A,B}(z::GradNumTup{N,A}, w::GradNumTup{N,B}) = GradientNumber(value(z)+value(w), add_tuples(grad(z), grad(w)))
+{N,T}(x::Real, z::GradNumTup{N,T}) = GradientNumber(x+value(z), grad(z))
+{N,T}(z::GradNumTup{N,T}, x::Real) = x+z

-{N,T}(z::GradNumTup{N,T}) = GradientNumber(-value(z), minus_tuple(grad(z)))
-{N,A,B}(z::GradNumTup{N,A}, w::GradNumTup{N,B}) = GradientNumber(value(z)-value(w), subtract_tuples(grad(z), grad(w)))
-{N,T}(x::Real, z::GradNumTup{N,T}) = GradientNumber(x-value(z), minus_tuple(grad(z)))
-{N,T}(z::GradNumTup{N,T}, x::Real) = GradientNumber(value(z)-x, grad(z))
...
function *{N,A,B}(z::GradNumTup{N,A}, w::GradNumTup{N,B})
    ...
    return GradientNumber(z_r*w_r, dus)
end

*{N,T}(x::Real, z::GradNumTup{N,T}) = GradientNumber(x*value(z), scale_tuple(x, grad(z)))
*{N,T}(z::GradNumTup{N,T}, x::Real) = x*z

# Division #
#----------#
/{N,T}(z::GradNumTup{N,T}, x::Real) = GradientNumber(value(z)/x, div_tuple_by_scalar(grad(z), x))

function /{N,T}(x::Real, z::GradNumTup{N,T})
    z_r = value(z)
    return GradientNumber(x/z_r, div_real_by_dus(-x, grad(z), z_r^2))
end
...
for f in (:^, :(NaNMath.pow))
    ...
    for R in (:Integer, :Rational, :Real)
        @eval function ($f){N,T}(z::GradNumTup{N,T}, x::($R))
            z_r = value(z)
            powval = x*($f)(z_r, x-1)
            return GradientNumber(($f)(z_r, x), scale_tuple(powval, grad(z)))
        end

        @eval function ($f){N,T}(x::($R), z::GradNumTup{N,T})
            z_r = value(z)
            logval = ($f)(x, z_r)*log(x)
            return GradientNumber(($f)(x, z_r), scale_tuple(logval, grad(z)))
        end
    end
...
for fsym in fad_supported_univar_funcs
    ...
    @eval function $(fsym){N,T}(z::GradNumTup{N,T})
        x = value(z)
        df = $dfexpr
        return GradientNumber($valexpr, scale_tuple(df, grad(z)))
    end
    ...
    if fsym in (:sin, :cos, :tan, 
        ...
        @eval function $(nan_fsym){N,T}(z::GradNumTup{N,T})
            ...
            return GradientNumber($(nan_fsym)(x), scale_tuple(df, grad(z)))
        end
...
function exp{N,T}(z::GradNumTup{N,T})
    df = exp(value(z))
    return GradientNumber(df, scale_tuple(df, grad(z)))
end